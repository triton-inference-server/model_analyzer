...
class ReportManager:
    ...
    def _build_summary_report(self, model_name, statistics):
        ...
        measurement_per_config = total_measurements // total_configurations \
            if total_configurations != 0 else 0
        ...
        summary.add_paragraph(
            f"For {model_name}, {measurement_per_config} "
            f"measurements for each of {total_configurations} total model "
            f"configurations were obtained. Curves corresponding "
            f"to the {num_best_configs} best model configurations are "
            "shown in the plots.")
