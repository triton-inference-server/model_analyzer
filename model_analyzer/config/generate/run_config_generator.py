...
class RunConfigGenerator(ConfigGeneratorInterface):
    ...
    def is_done(self):
        ...
        # FIXME: need to walk each model config separately
    ...
    def next_config(self):
        ...
        while not pacg.is_done():
            ...
            mcgs = [
                ModelConfigGenerator(self._config, model, self._client)
                for model in self._models
            ]

            while not mcgs[0].is_done():
                model_configs = [mcg.next_config() for mcg in mcgs]

                run_config = self._generate_run_config(model_configs,
                                                       perf_analyzer_config)

                if pacg.is_done() and mcgs[0].is_done():
                    self._is_done = True
    ...
    #         # FIXME: create list of model names
